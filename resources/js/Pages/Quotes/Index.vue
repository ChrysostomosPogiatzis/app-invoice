    ...
    <AuthenticatedLayout>
            ...
            <div class="flex flex-col lg:flex-row justify-between items-center w-full gap-4">
                ...
                <div class="flex flex-wrap items-center gap-3 w-full lg:w-auto">
                    ...
                    <div class="relative flex-grow lg:flex-grow-0">
                        <input v-model="search" type="text" placeholder="Search quote #" class="bg-white border border-slate-200 rounded-lg py-2 pl-10 pr-4 text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none w-full lg:w-64 transition-all" />
                    ...
                    <Link :href="route('quotes.create')" class="bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-indigo-700 transition-colors whitespace-nowrap">
                        New Quote
                    </Link>
                </div>
            </div>
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <table class="w-full text-left">
                <thead class="bg-slate-50 border-b border-slate-200">
                    <tr>
                        ...
                        <th 
                            class="px-6 py-4 text-xs font-semibold text-slate-600 uppercase tracking-wider cursor-pointer hover:bg-slate-100 transition-colors select-none"
                            @click="triggerSort('quote_number')"
                        >
                            <span class="flex items-center gap-1">
                                Quote #
                                <span class="text-slate-400 text-[10px]">{{ sortIcon('quote_number') }}</span>
                            </span>
                        </th>
                        ...
                        <th 
                            class="px-6 py-4 text-xs font-semibold text-slate-600 uppercase tracking-wider text-center cursor-pointer hover:bg-slate-100 transition-colors select-none"
                            @click="triggerSort('valid_until')"
                        >
                            <span class="flex items-center justify-center gap-1">
                                Valid Until
                                <span class="text-slate-400 text-[10px]">{{ sortIcon('valid_until') }}</span>
                            </span>
                        </th>
                        <th 
                ...
                <tbody class="divide-y divide-slate-100">
                    <tr v-for="quote in quotes.data" :key="quote.id" class="hover:bg-slate-50/50 transition-colors">
                        <td class="px-6 py-4 text-sm text-slate-500 tabular-nums">
                            {{ new Date(quote.date).toLocaleDateString('en-GB') }}
                        </td>
                        <td class="px-6 py-4">
                            <Link :href="route('quotes.show', quote)" class="text-sm font-bold text-slate-900 hover:text-indigo-600 transition-colors">
                                {{ quote.quote_number }}
                            </Link>
                        </td>
                        <td class="px-6 py-4">
                            <div v-if="quote.contact">
                                <Link :href="route('contacts.show', quote.contact_id)" class="text-sm font-medium text-slate-800 hover:text-indigo-600">{{ quote.contact.name }}</Link>
                                <div class="text-[11px] text-slate-400 font-medium line-clamp-1">{{ quote.contact.company_name || 'Individual' }}</div>
                            </div>
                        </td>
                        <td class="px-6 py-4 text-center">
                            <span :class="getStatusStyle(quote.status)" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border capitalize">
                                {{ quote.status }}
                            </span>
                        </td>
                        <td class="px-6 py-4 text-center text-sm text-slate-500 tabular-nums">
                            {{ new Date(quote.valid_until).toLocaleDateString('en-GB') }}
                        </td>
                        <td class="px-6 py-4 text-right">
                            <span class="text-sm font-bold text-slate-900 tabular-nums">€{{ parseFloat(quote.grand_total_gross || 0).toFixed(2) }}</span>
                        </td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end gap-2">
                                <Link :href="route('quotes.show', quote)" class="p-2 text-slate-400 hover:text-slate-900 transition-colors">
                                ...
                                <button v-if="quote.status === 'draft'" @click="deleteQuote(quote.id)" class="p-2 text-slate-300 hover:text-rose-600 transition-colors">
                    ...
                    <tr v-if="quotes.data.length === 0">
                        <td colspan="7" class="px-6 py-20 text-center text-sm text-slate-400 italic">No quotes found.</td>
                    </tr>
                </tbody>
            </table>
            ...
            <div class="px-6 py-4 bg-slate-50 border-t border-slate-200 flex items-center justify-between">
                <span class="text-xs text-slate-500 font-medium">Showing {{ quotes.data.length }} of {{ quotes.total }} quotes</span>
                <div class="flex gap-2">
                    <Link v-for="link in quotes.links" :key="link.label" :href="link.url || '#'" 
                          v-html="link.label"
                          :class="[

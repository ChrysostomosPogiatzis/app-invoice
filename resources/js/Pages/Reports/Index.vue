...
// CPA Export Range (Default: Last 3 Months)
const today = new Date();
const threeMonthsAgo = new Date();
threeMonthsAgo.setMonth(today.getMonth() - 3);

const dateFrom = ref(threeMonthsAgo.toISOString().split('T')[0]);
const dateTo = ref(today.toISOString().split('T')[0]);

const downloadAccountantExport = () => {
    window.location.href = route('reports.accountant.export') + `?date_from=${dateFrom.value}&date_to=${dateTo.value}`;
};
...
const downloadMonthlyPayroll = () => {
    window.location.href = route('reports.monthly.payroll') + '?month=' + selectedMonth.value;
};
...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="max-w-6xl mx-auto space-y-8">
            ...
            <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div class="p-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <!-- Payroll PDF -->
                        <div class="relative overflow-hidden border border-slate-200 rounded-xl p-7 group hover:border-emerald-300 hover:shadow-md transition-all bg-gradient-to-br from-white to-emerald-50/30">
                            <div class="absolute -top-6 -right-6 w-24 h-24 bg-emerald-100 rounded-full opacity-30 group-hover:opacity-50 transition-opacity"></div>
                            <div class="flex items-start gap-4 mb-5">
                                <div class="w-12 h-12 bg-emerald-600 text-white rounded-xl flex items-center justify-center shadow-lg flex-shrink-0">
                                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <div>
                                    <h4 class="text-base font-bold text-slate-900">Payroll Liabilities</h4>
                                    <p class="text-xs text-slate-500 mt-0.5 leading-relaxed">Summary of Social Insurance, GESI, and PAYE Tax for all staff. Essential for monthly department payments.</p>
                                </div>
                            </div>
                            <div class="flex items-center justify-between pt-4 border-t border-slate-100">
                                <div class="flex items-center gap-2 text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                                ...
                                <button
                                    @click="downloadMonthlyPayroll"
                                    class="flex items-center gap-2 bg-emerald-600 text-white text-xs font-bold px-5 py-2.5 rounded-lg hover:bg-emerald-700 transition-colors shadow-sm"
                                >
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                                    </svg>
            ...
            <!-- Accountant / CPA Export -->
            <div class="bg-indigo-600 rounded-xl border border-indigo-500 shadow-xl overflow-hidden text-white">
                <div class="px-8 py-6 border-b border-indigo-500/50 bg-indigo-700/30 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h3 class="text-sm font-bold uppercase tracking-widest text-white">Tax Accountant Package</h3>
                            <p class="text-xs text-indigo-100 opacity-80 mt-0.5">Generate a ZIP file with all documents for your fiscal declaration.</p>
                        </div>
                    </div>
                </div>

                <div class="p-8 flex flex-col md:flex-row items-end gap-6 bg-gradient-to-br from-indigo-600 to-indigo-800">
                    <div class="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-4 w-full">
                        <div>
                            <label class="block text-[10px] font-bold text-indigo-200 uppercase tracking-widest mb-2">Export Start Date</label>
                            <input
                                v-model="dateFrom"
                                type="date"
                                class="w-full bg-indigo-900/40 border border-indigo-400/30 text-white text-sm rounded-lg px-4 py-3 focus:ring-2 focus:ring-white/30 focus:border-white outline-none transition-all tabular-nums"
                            />
                        </div>
                        <div>
                            <label class="block text-[10px] font-bold text-indigo-200 uppercase tracking-widest mb-2">Export End Date</label>
                            <input
                                v-model="dateTo"
                                type="date"
                                class="w-full bg-indigo-900/40 border border-indigo-400/30 text-white text-sm rounded-lg px-4 py-3 focus:ring-2 focus:ring-white/30 focus:border-white outline-none transition-all tabular-nums"
                    ...
                    <button
                        @click="downloadAccountantExport"
                        class="w-full md:w-auto flex items-center justify-center gap-3 bg-white text-indigo-600 text-sm font-black px-8 py-3.5 rounded-xl hover:bg-slate-100 transition-all shadow-lg shadow-indigo-950/20 active:scale-[0.98]"
                    >
                ...
                <div class="px-8 py-4 bg-indigo-950/20 border-t border-indigo-500/20 flex gap-6 overflow-x-auto no-scrollbar">
                    <div class="flex items-center gap-2 flex-shrink-0">
                        ...
                        <span class="text-[10px] font-bold text-indigo-100/70 uppercase">Individual Invoice PDFs</span>
                    </div>
                    <div class="flex items-center gap-2 flex-shrink-0">
                        <svg class="w-4 h-4 text-emerald-400" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" /></svg>
                        <span class="text-[10px] font-bold text-indigo-100/70 uppercase">Invoicing & Expense Summaries</span>
                    </div>
                    <div class="flex items-center gap-2 flex-shrink-0">
                        <svg class="w-4 h-4 text-emerald-400" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" /></svg>
                        <span class="text-[10px] font-bold text-indigo-100/70 uppercase">Detail Banking Log</span>
                    </div>
                </div>
            </div>

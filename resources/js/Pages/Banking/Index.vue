...
const viewingTx    = ref<any | null>(null);
...
const providerMeta: Record<string, { label: string; color: string; bg: string; dot: string }> = {
    ...
    eurobank:   { label: 'Eurobank',           color: 'text-rose-700',    bg: 'bg-rose-50 border-rose-100',    dot: 'bg-rose-500' },
    boc:        { label: 'Bank of Cyprus',    color: 'text-blue-700',    bg: 'bg-blue-50 border-blue-100',    dot: 'bg-blue-500' },
};
...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="space-y-8">
            ...
            <div v-if="connections.length" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                >
                    <div class="flex items-start justify-between">
                        <div>
                            ...
                            <div class="text-[10px] text-slate-400 mt-1">
                                {{ conn.transactions_count }} transactions synced
                                <span v-if="conn.provider === 'boc'" class="block text-emerald-600 font-bold mt-0.5">
                                    Autosync: 07:00 & 14:00
                                </span>
                            </div>
                        </div>
                    ...
                    <div v-if="conn.accounts && conn.accounts.length" class="space-y-3 pt-3 border-t border-slate-100">
                        <div v-for="acc in conn.accounts" :key="acc.account_number" class="flex items-center justify-between p-2 rounded-lg bg-slate-50 border border-slate-100/50">
                            <div class="flex flex-col min-w-0">
                                <div class="flex items-center gap-2">
                                    <span class="text-[9px] font-bold text-slate-400 uppercase leading-none">{{ acc.name || 'Account' }}</span>
                                    <span class="text-[9px] font-bold px-1.5 py-0.5 rounded bg-white border border-slate-200 text-slate-500">{{ acc.currency }}</span>
                                </div>
                                <span class="text-[10px] font-mono text-slate-600 mt-1 truncate" :title="acc.iban || acc.account_number">{{ acc.iban || acc.account_number }}</span>
                            </div>
                            <div v-if="acc.balance !== undefined" class="text-right ml-2 shrink-0">
                                <div class="text-[9px] font-bold text-slate-400 uppercase leading-none mb-1">Balance</div>
                                <div class="text-xs font-bold text-emerald-600 tabular-nums">{{ formatAmount(acc.balance) }}</div>
                            </div>
                        </div>
                    </div>

                    <div class="flex items-center gap-2 pt-2 border-t border-slate-100">
                        <!-- Connect Button (for OAuth2 providers) -->
                        <a v-if="['boc', 'eurobank'].includes(conn.provider)"
                           :href="route('banking.connect', conn.id)"
                           class="flex-1 flex items-center justify-center gap-1.5 bg-indigo-600 text-white text-[11px] font-bold py-2 px-3 rounded-lg hover:bg-indigo-700 transition-colors">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" stroke-width="2" stroke-linecap="round"/></svg>
                            Connect
                        </a>

                        <button v-else @click="syncConnection(conn.id)" :disabled="syncingId === conn.id"
                        ...
                        <button v-if="['eurobank'].includes(conn.provider)" @click="syncConnection(conn.id)" :disabled="syncingId === conn.id"
                            class="p-2 text-slate-400 hover:text-slate-900 transition-colors" title="Sync Transactions">
                            <svg :class="{ 'animate-spin': syncingId === conn.id }" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" stroke-width="2" stroke-linecap="round"/>
                            </svg>
                        </button>
                        <button v-if="['eurobank'].includes(conn.provider)" @click="() => router.post(route('banking.refresh-balances', conn.id), {}, { preserveScroll: true })"
                            class="p-2 text-slate-400 hover:text-emerald-600 transition-colors" title="Refresh Balances">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path d="M12 8c-1.657 0-3 1.343-3 3s1.343 3 3 3 3-1.343 3-3-1.343-3-3-3z" stroke-width="2" stroke-linecap="round"/>
                                <path d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" stroke-width="2" stroke-linecap="round"/>
                            </svg>
                        </button>
                        ...
                        <tr v-for="tx in transactions.data" :key="tx.id" class="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td class="px-5 py-3 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <button @click="viewingTx = tx" class="p-1.5 text-slate-400 hover:text-indigo-600 transition-colors" title="View details">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" stroke-width="2" stroke-linecap="round"/><path d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" stroke-width="2" stroke-linecap="round"/></svg>
                                    </button>
        ...
        <div v-if="viewingTx" class="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div class="absolute inset-0 bg-black/40 backdrop-blur-sm" @click="viewingTx = null"></div>
            <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl p-0 z-10 flex flex-col max-h-[90vh]">
                <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                    <div>
                        <h3 class="text-sm font-bold text-slate-900 uppercase tracking-widest">Transaction Details</h3>
                        <p class="text-[10px] text-slate-400 mt-0.5">ID: {{ viewingTx.external_id }}</p>
                    </div>
                    <button @click="viewingTx = null" class="text-slate-400 hover:text-slate-600">
                ...
                <div class="flex-1 overflow-y-auto p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="bg-slate-50 p-3 rounded-lg border border-slate-100">
                            <div class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Date & Time</div>
                            <div class="text-sm font-medium text-slate-800">{{ new Date(viewingTx.transaction_date).toLocaleString('en-GB') }}</div>
                        </div>
                        <div class="bg-slate-50 p-3 rounded-lg border border-slate-100">
                            <div class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Amount</div>
                            <div class="text-sm font-bold text-indigo-600">{{ formatAmount(viewingTx.amount) }} {{ viewingTx.currency }}</div>
                        </div>
                    </div>

                    <!-- Description -->
                    <div v-if="viewingTx.description" class="bg-indigo-50/50 p-4 rounded-xl border border-indigo-100/50">
                        <div class="text-[10px] font-bold text-indigo-400 uppercase tracking-wider mb-1">Description</div>
                        <div class="text-sm text-indigo-900 font-medium">{{ viewingTx.description }}</div>
                    </div>

                    <!-- Provider Raw Payload -->
                    <div>
                        <div class="flex items-center gap-2 mb-3">
                            <span class="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Metadata / Raw Payload</span>
                            <div class="h-px bg-slate-100 flex-1"></div>
                        </div>
                        <div class="bg-slate-900 rounded-xl p-4 overflow-x-auto">
                            <pre class="text-[11px] text-indigo-300 font-mono leading-relaxed">{{ JSON.stringify(viewingTx.raw_payload, null, 2) }}</pre>
                        </div>
                    </div>
                </div>

                <div class="px-6 py-4 bg-slate-50 border-t border-slate-100 text-right">
                    <button @click="viewingTx = null" class="bg-white border border-slate-200 text-slate-700 text-xs font-bold py-2 px-6 rounded-lg hover:bg-slate-50 transition-colors shadow-sm">
                        Close
                    </button>
                </div>
            </div>

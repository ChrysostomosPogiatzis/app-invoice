                ...
                <table class="w-full text-left">
                    <thead class="bg-slate-50 border-b border-slate-200">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-xs font-semibold text-slate-600 uppercase tracking-wider text-right">Tax / ER-SI</th>
                            <th class="px-6 py-4 text-xs font-semibold text-slate-600 uppercase tracking-wider text-right">Total Outflow</th>
                        ...
                        <tr v-for="exp in expenses.data" :key="exp.id" class="hover:bg-slate-50/50 transition-colors group">
                            ...
                            <td class="px-6 py-4 text-right tabular-nums">
                                <span v-if="exp.is_payroll" class="text-indigo-400 text-xs font-medium">
                                    +€{{ (Number(exp.si_employer || 0) + Number(exp.gesi_employer || 0) + Number(exp.provident_employer || 0) + Number(exp.redundancy_amount || 0) + Number(exp.training_amount || 0) + Number(exp.cohesion_amount || 0) + Number(exp.holiday_amount || 0)).toFixed(2) }}
                                </span>
                                <span v-else-if="exp.vat_amount && Number(exp.vat_amount) > 0" class="text-indigo-600 font-semibold text-xs">+€{{ Number(exp.vat_amount).toFixed(2) }}</span>
                                <span v-else class="text-slate-300 text-[8px] uppercase font-bold tracking-widest italic leading-3">No Extra Tax</span>
                            </td>
                            <td class="px-6 py-4 text-right tabular-nums bg-indigo-50/20">
                                <div class="font-black text-indigo-900 text-base">€{{ (Number(exp.amount) + Number(exp.vat_amount || 0) + Number(exp.si_employer || 0) + Number(exp.gesi_employer || 0) + Number(exp.provident_employer || 0) + Number(exp.redundancy_amount || 0) + Number(exp.training_amount || 0) + Number(exp.cohesion_amount || 0) + Number(exp.holiday_amount || 0)).toFixed(2) }}</div>
                                <div class="text-[8px] text-indigo-400 font-bold uppercase">Total Book Cost</div>
                            </td>

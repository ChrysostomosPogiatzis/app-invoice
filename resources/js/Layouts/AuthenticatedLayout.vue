...
const navItems = [
    ...
    { name: 'Payments', route: 'payments.index', icon: 'M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z', active: 'payments.*' },
    ...
    { name: 'Staff', route: 'staff-members.index', icon: 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z', active: 'staff-members.*' },
...
const filteredWorkspaces = computed(() => {
    if (!workspaceSearch.value) return user.value.workspaces || [];
    const search = workspaceSearch.value.toLowerCase();
    return (user.value.workspaces || []).filter(w => w.name.toLowerCase().includes(search));
});
</script>
...
<template>
    <div class="min-h-screen bg-slate-50 flex font-sans selection:bg-indigo-100 selection:text-indigo-900">
        ...
        <aside class="w-64 bg-white border-r border-slate-200 fixed inset-y-0 left-0 z-50 flex flex-col">
            ...
            <!-- Workspace Switcher -->
            <div v-if="(user.workspaces?.length ?? 0) > 1" class="px-4 py-4 border-b border-slate-100 bg-slate-50/50">
                <label class="block text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2 px-2">Active Node Search</label>
                <div class="relative mb-3 group">
                    <input 
                        v-model="workspaceSearch"
                        type="text" 
                        class="w-full bg-slate-100 border-none rounded-xl px-4 py-2 text-[11px] font-bold text-slate-700 focus:ring-4 focus:ring-indigo-500/10 transition-all placeholder:text-slate-400"
                        placeholder="Type node name..."
                    />
                    <div class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-300">
                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"></path></svg>
                    </div>
                </div>
                <div class="relative group">
                    <select 
                        :value="workspace?.id"
                        @change="(e) => router.post(route('workspaces.switch'), { workspace_id: (e.target as HTMLSelectElement).value })"
                        class="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-xs font-bold text-slate-700 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all cursor-pointer appearance-none shadow-sm"
                    >
                        <option v-for="w in filteredWorkspaces" :key="w.id" :value="w.id">{{ w.name }}</option>
                        <option v-if="filteredWorkspaces.length === 0" disabled>No nodes found...</option>
                    </select>
        ...
        <main class="flex-grow pl-64">
            <!-- Trial Banner -->
            <div v-if="workspace && workspace.trial_days_left <= 7 && !workspace.trial_expired && workspace.tier !== 'enterprise'" 
                :class="[workspace.trial_days_left <= 2 ? 'bg-amber-50 border-amber-200 text-amber-800' : 'bg-indigo-50 border-indigo-200 text-indigo-800']"
                class="px-8 py-3 border-b flex items-center justify-between"
            >
                <div class="flex items-center gap-3">
                    <div :class="[workspace.trial_days_left <= 2 ? 'bg-amber-100 text-amber-600' : 'bg-indigo-100 text-indigo-600']" class="w-8 h-8 rounded-lg flex items-center justify-center font-bold text-xs">
                        {{ workspace.trial_days_left }}
                    </div>
                    <div>
                        <p class="text-xs font-bold leading-none">{{ workspace.trial_days_left }} Days Remaining in Trial</p>
                        <p class="text-[10px] opacity-70 mt-1 font-medium italic">Your {{ workspace.tier === 'starter' ? 'Freelancer' : 'Small Biz' }} node will expire in {{ workspace.trial_days_left }} days.</p>
                    </div>
                </div>
                <Link :href="route('settings.edit')" class="bg-white border border-current/20 px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-white/50 transition-colors">
                    Upgrade License
                </Link>
            </div>

            <!-- Expired Banner -->
            <div v-if="workspace && workspace.trial_expired && workspace.tier !== 'enterprise'" 
                class="bg-rose-600 text-white px-8 py-3 flex items-center justify-between"
            >
                <div class="flex items-center gap-3">
                    ...
                    <p class="text-xs font-bold tracking-tight uppercase">License Expired: Node Locked</p>
                </div>
                <Link :href="route('settings.edit')" class="bg-white text-rose-600 px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-rose-50 transition-colors shadow-lg">
                    Re-Activate Node
                </Link>
            </div>

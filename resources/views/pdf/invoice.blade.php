...
<head>
    <meta charset="utf-8">
    <title>{{ ($isQuote ?? false) ? 'Quotation' : 'Tax Invoice' }} #{{ $invoice->invoice_number ?? $invoice->quote_number }}</title>
    <style>
        @page {
            margin: 30px;
        }
    ...
    <table class="header-table">
        <tr>
            ...
            <td class="invoice-type">
                <h1 class="title">{{ ($isQuote ?? false) ? 'Quotation' : 'Tax Invoice' }}</h1>
                <div style="margin-top: 15px;">
                    <strong>No:</strong> {{ $invoice->invoice_number ?? $invoice->quote_number }}<br> <strong>Date:</strong>
                    {{ \Carbon\Carbon::parse($invoice->date)->format('d/m/Y') }}<br>
                    @if(!($isQuote ?? false))
                        <strong>V.A.T. Reg:</strong> {{ $invoice->workspace->vat_number }}<br> <strong>T.I.C:</strong> {{ $invoice->workspace->tax_id }}
                    @else
                        <strong>Valid Until:</strong> {{ \Carbon\Carbon::parse($invoice->valid_until)->format('d/m/Y') }}
                    @endif
                </div>
            </td>
        </tr>

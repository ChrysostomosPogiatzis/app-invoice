...
<body>
    ...
    <table class="header-table">
        <tr>
            <td class="company-brand">
                @if($quote->workspace->logo_url)
                    <img src="{{ public_path($quote->workspace->logo_url) }}"
                        style="max-height: 60px; margin-bottom: 10px;">
                @endif
                <div class="company-name">{{ $quote->workspace->company_name }}</div>
                <div style="font-size: 10px; color: #475569;">
                    {{ $quote->workspace->address }}<br> Tel: {{ $quote->workspace->phone_number }} | Email:
                    {{ $quote->workspace->email }}
                </div>
            </td>
            <td class="invoice-type">
                <h1 class="title">Quote</h1>
                <div style="margin-top: 15px;">
                    <strong>No:</strong> {{ $quote->quote_number }}<br> <strong>Date:</strong>
                    {{ \Carbon\Carbon::parse($quote->date)->format('d/m/Y') }}<br> <strong>V.A.T. Reg:</strong>
                    {{ $quote->workspace->vat_number }}<br> <strong>T.I.C:</strong> {{ $quote->workspace->tax_id }}
                </div>
            </td>
        </tr>
    </table>
    ...
    <table class="info-grid">
        <tr>
            <td class="info-col">
                ...
                <div style="font-size: 14px; font-weight: 700;">{{ $quote->contact->name }}</div>
                @if($quote->contact->address)
                    <div>{{ $quote->contact->address }}</div>
                @endif
                @if($quote->contact->vat_number)
                    <div>VAT: {{ $quote->contact->vat_number }}</div>
                @endif
            </td>
            <td class="info-col">
                <span class="label">Payment Details</span>
                <div><strong>Bank:</strong> IBAN {{ $quote->workspace->iban }}</div>
                <div><strong>BIC/SWIFT:</strong> {{ $quote->workspace->bic }}</div> @if($quote->notes)
                <div style="margin-top: 10px; color: #64748b;">Ref: {{ $quote->notes }}</div> @endif
            </td>
        </tr>
    </table>
    ...
    <table class="totals-table">
        <tr>
            ...
            <td class="text-right">{{ number_format($quote->subtotal_net, 2) }}</td>
        </tr>
        <tr>
            <td style="color: #64748b;">V.A.T. Amount</td>
            <td class="text-right">{{ number_format($quote->grand_total_gross - $quote->subtotal_net, 2) }}</td>
        </tr>
        <tr class="grand-total">
            <td>Grand Total (EUR)</td>
            <td class="text-right">€{{ number_format($quote->grand_total_gross, 2) }}</td>
        </tr>
        @if($quote->amount_paid > 0)
            <tr>
                <td style="color: #10b981; font-weight: 700;">Amount Paid</td>
                <td class="text-right" style="color: #10b981;">-{{ number_format($quote->amount_paid, 2) }}</td>
            </tr>
        @endif
        <tr style="border-top: 2px solid #0f172a;">
            <td style="font-weight: 700;">Balance Due</td>
            <td class="text-right" style="font-weight: 700;">€{{ number_format($quote->balance_due, 2) }}</td>
        </tr>
    </table>

    @if($quote->notes)
    <div style="margin-top: 30px; border-left: 3px solid #cbd5e1; padding-left: 15px;">
        <span class="label">Quote Notes</span>
        <div style="font-size: 10px; color: #1e293b;">{!! nl2br(e($quote->notes)) !!}</div>
    </div>
    @endif
    ...
    <table class="signature-section">
        <tr>
            ...
            <td class="text-right">
                <div class="sig-box" style="margin-left: auto;">Buyer Signature: {{ $quote->contact->name }}</div>
            </td>
        </tr>
    </table>

    <div style="text-align: center; margin-top: 50px; font-size: 8px; color: #cbd5e1;">
        This document is a Quote in accordance with Cyprus VAT Legislation. </div>

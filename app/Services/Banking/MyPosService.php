...
class MyPosService
{
    ...
    {
        $isDemo = $connection->credential('is_demo', config('services.mypos.is_demo', false));
        $clientId = config('services.mypos.partner_client_id', '');
    ...
    {
        $clientId = config('services.mypos.partner_client_id');
        $clientSecret = config('services.mypos.partner_client_secret');
    ...
    private function getSessionId(string $bearerToken): string
    {
        ...
        $partnerId = config('services.mypos.partner_id');
        $appId = config('services.mypos.application_id');
    ...
    public function syncTransactions(string $dateFrom, string $dateTo): int
    {
        ...
        $sessionId = $this->isPartnerFlow ? $this->getSessionId($bearerToken) : null;
        ...
        $rows = $response->json('data') ?? $response->json('transactions') ?? $response->json('list') ?? $response->json('items') ?? [];
        $count = 0;
        ...
        foreach ($rows as $tx) {
            ...
            BankTransaction::updateOrCreate(
                ...
                [
                    ...
                    'type' => $tx['transactionType'] ?? $tx['transaction_type'] ?? $tx['type'] ?? null,
                    'amount' => $tx['amount'] ?? $tx['transaction_amount'] ?? 0,
                    'currency' => $tx['currency'] ?? $tx['transaction_currency'] ?? 'EUR',
                    'status' => $tx['status'] ?? null,
                    'card_type' => $tx['cardBrand'] ?? $tx['card_type'] ?? null,
                    'card_last4' => $tx['cardLastFour'] ?? $tx['card_last4'] ?? $tx['pan'] ?? null,
    ...
    /**
     * Fetch all associated accounts/wallets and their details (including balance via MT940)
     */
    public function getAccounts(): array
    {
        ...
        if ($this->isPartnerFlow && $sessionId) {
            $headers['X-Session'] = $sessionId;
            $headers['X-Partner-Id'] = config('services.mypos.partner_id');
            $headers['X-Application-Id'] = config('services.mypos.application_id');
        }

        $response = Http::withHeaders($headers)
            ->get("{$this->baseUrl}/accounting/v1/accounts");
        ...
        // Try to get balance for each account using the MT940 trick
        foreach ($items as &$item) {
            $item['balance'] = $this->getBalanceFromStatement($item['account_number']);
        }
    ...
    /**
     * Fetch the balance from the MT940 statement export as a fallback
     */
    public function getBalanceFromStatement(string $accountNumber): ?float
    {
        try {
            $bearerToken = $this->getBearerToken();
            $sessionId = $this->isPartnerFlow ? $this->getSessionId($bearerToken) : null;
            ...
            // Using MT940 export to get the balance from :62F: tag
            $response = Http::withHeaders($headers)
                ->timeout(10)
                ->post("{$this->baseUrl}/accounting/v1/accounts/statements", [
                    'account_number' => $accountNumber,
                    'document_type' => 'MT940MultiCash',
                    'date' => now()->subDay()->format('Y-m-d\TH:i:s\Z'),
                ]);

            if ($response->successful()) {
                $body = $response->body();
                // Match closing balance tag :62F: or :62M:
                // Pattern: :62F: followed by C/D, then 6 digits (date), then 3 chars (currency), then Amount
                if (preg_match('/:62[F|M]:[C|D]\d{6}[A-Z]{3}([\d,.]+)/', $body, $matches)) {
                    $balanceStr = str_replace(',', '.', $matches[1]);
                    return (float)$balanceStr;
                }
            }
        } catch (\Exception $e) {
            \Log::warning("MyPos Balance fetching failed for account {$accountNumber}: " . $e->getMessage());
        }

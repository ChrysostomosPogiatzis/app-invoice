        ...
        return [
            ...
            'auth' => [
                'user' => $user ? [
                    ...
                    'workspaces' => $user->isSuperAdmin() 
                        ? \App\Models\Workspace::select('id', 'company_name as name')->get()
                        : $user->workspaces->map(fn($w) => [
                            'id' => $w->id,
                            'name' => $w->company_name,
                        ]),
                ] : null,
            ...
            'workspace' => $workspace ? [
                ...
                'features' => array_merge(
                    $workspace->features()->pluck('is_enabled', 'feature_name')->toArray(),
                    collect(\App\Services\TierService::getLimits($workspace->tier)['features'])
                        ->mapWithKeys(fn($f) => [$f => true])
                        ->toArray()
                ),
                'trial_expired' => \App\Services\TierService::isTrialExpired($workspace),
                'trial_days_left' => \App\Services\TierService::getTrialRemainingDays($workspace),
                'tier' => $workspace->tier,
            ] : null,
        ];
    }

...
class ProductController extends Controller
{
    ...
    {
        $product = \App\Models\Product::where('workspace_id', $this->getWorkspaceId())->findOrFail($id);
        ...
        if ($validated['direction'] === 'out' && $product->current_stock < $validated['quantity']) {
            return response()->json(['error' => 'Insufficient stock.'], 422);
        }

        $product->current_stock += $validated['direction'] === 'in'
            ? $validated['quantity']
            : -$validated['quantity'];
        $product->save();

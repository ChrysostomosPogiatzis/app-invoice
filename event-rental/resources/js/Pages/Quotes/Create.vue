...
const form = useForm({
    ...
    items: [
        { product_id: null as number | null, description: '', quantity: 1, unit_price_net: 0, vat_rate: 19 }
    ],
    discount: 0,
    notes: '',
    terms: ''
...
const calculateGross = (index: number) => {
    const item = form.items[index];
    const net = Number(item.unit_price_net) || 0;
        ...
        <template #header>
            <div class="flex justify-between items-center w-full">
                <div class="flex items-center gap-4">
                    <Link :href="route('quotes.index')" class="text-gray-400 hover:text-gray-600">
                    ...
                    <h2 class="text-xl font-semibold text-gray-800">New Quote #{{ workspace.quote_prefix || 'Q-' }}{{ String(workspace.next_quote_number).padStart(4, '0') }}</h2>
                </div>
        ...
        <div class="py-8 max-w-5xl mx-auto px-4">
            <form id="quote-form" @submit.prevent="submit" class="space-y-6">
                ...
                <div class="flex justify-end gap-4">
                    <Link :href="route('quotes.index')" class="px-6 py-2 text-sm font-medium text-gray-500 hover:text-gray-700">Discard</Link>

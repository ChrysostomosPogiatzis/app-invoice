...
class SettingsActivity : SimpleActivity() {
    ...
    private fun setupErpUrl() {
        binding.settingsErpUrl.text = config.erpUrl.ifEmpty { getString(R.string.erp_server_url_hint) }
        binding.settingsErpUrlHolder.setOnClickListener {
            val editText = android.widget.EditText(this)
            editText.setText(config.erpUrl)
            androidx.appcompat.app.AlertDialog.Builder(this)
                .setTitle(R.string.erp_server_url)
                .setView(editText)
                .setPositiveButton(android.R.string.ok) { _, _ ->
                    config.erpUrl = editText.text.toString().trim().removeSuffix("/")
                    binding.settingsErpUrl.text = config.erpUrl.ifEmpty { getString(R.string.erp_server_url_hint) }
                }
    ...
    private fun setupErpToken() {
        binding.settingsErpToken.text = if (config.erpToken.isEmpty()) getString(R.string.erp_api_token_hint) else "********"
        binding.settingsErpTokenHolder.setOnClickListener {
            val editText = android.widget.EditText(this)
            editText.setText(config.erpToken)
            androidx.appcompat.app.AlertDialog.Builder(this)
                .setTitle(R.string.erp_api_token)
                .setView(editText)
                .setPositiveButton(android.R.string.ok) { _, _ ->
                    config.erpToken = editText.text.toString().trim()
                    binding.settingsErpToken.text = if (config.erpToken.isEmpty()) getString(R.string.erp_api_token_hint) else "********"
                }
                .setNegativeButton(android.R.string.cancel, null)
                .show()
